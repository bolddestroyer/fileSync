...
#Log file
def log_file(dir_src, dir_trg, log_lbl_processResult, stat_totalFiles, stat_unchangedFiles, stat_newFiles, log_lbl_newFiles, stat_replacedFiles, log_lbl_replacedFiles, stat_nonExistInSrc, log_lbl_nonExistInSrc, log_dir, log_rundate, processed_files, total_files, processing_duration):
    ...
    with open(log_dir, 'w', encoding='utf-8') as log_file:
        ...
        log_file.write(f'Processed files: {processed_files}/{total_files}\n')
...
def custom_messagebox(log_dir, log_lbl_processResult, log_rundate, processed_files, total_files, processing_duration):
    ...
    lbl_proc_files = tkinter.Label(msg_box, text=f'Processed files: {processed_files}/{total_files}')
    lbl_proc_files.grid(row=1, column=0, columnspan=2)

    lbl_proc_duration = tkinter.Label(msg_box, text=f'Processing duration: {processing_duration:.2f} seconds')
    lbl_proc_duration.grid(row=2, column=0, columnspan=2)

    lbl_runtime = tkinter.Label(msg_box, text=f'Synchronization run on {log_rundate}')
    lbl_runtime.grid(row=3, column=0, columnspan=2)
    ...
    btn_open_log.grid(row=4, column=1, padx=10, pady=10)
